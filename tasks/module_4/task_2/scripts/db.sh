...
function add {
    ...
    if [[ "$?" == 1 ]];
    then
      echo "Name must only contain latin letters. Try again."
      exit 1
    fi

    read -p "Enter the user role: " role

    validateLatinLetters $role
    
    if [[ "$?" == 1 ]]
    then
      echo "Role must only contains latin letters. Try again."
      exit 1
    fi
...
inverseParam="$2"
function list {
    if [[ $inverseParam == "inverse" ||  $inverseParam == "--inverse" ]] 
    then
